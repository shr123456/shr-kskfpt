package com.dkt.controller.equhighpc;
 ...
 */
@Controller
@RequestMapping(value="/equhighpc")
public class EquHighPCController extends BaseController {
	
	String menuUrl = "equhighpc/list.do"; //菜单地址(权限用)
	...
	public ModelAndView save() throws Exception{
		logBefore(logger, Jurisdiction.getUsername()+"新增EquHighPC");
	...
	public void delete(PrintWriter out) throws Exception{
		logBefore(logger, Jurisdiction.getUsername()+"删除EquHighPC");
		if(!Jurisdiction.buttonJurisdiction(menuUrl, "del")){return;} //校验权限
		PageData pd = new PageData();
		pd = this.getPageData();
		equhighpcService.delete(pd);
		out.write("success");
		out.close();
	}
	...
	public ModelAndView edit() throws Exception{
		logBefore(logger, Jurisdiction.getUsername()+"修改EquHighPC");
	...
	public ModelAndView list(Page page) throws Exception{
		logBefore(logger, Jurisdiction.getUsername()+"列表EquHighPC");
		...
		page.setPd(pd);
		List<PageData>	varList = equhighpcService.list(page);	//列出EquHighPC列表
		mv.setViewName("equhighpc/equhighpc_list");
	...
	public ModelAndView goAdd()throws Exception{
		...
		PageData pd = new PageData();
		pd = this.getPageData();
		mv.setViewName("equhighpc/equhighpc_edit");
...
//		User user = (User)session.getAttribute(Const.SESSION_USER);//当前用户
	...
	public ModelAndView goEdit()throws Exception{
		...
		PageData pd = new PageData();
		pd = this.getPageData();
		pd = equhighpcService.findById(pd);	//根据ID读取
		mv.setViewName("equhighpc/equhighpc_edit");
	...
	public Object deleteAll() throws Exception{
		logBefore(logger, Jurisdiction.getUsername()+"批量删除EquHighPC");
		...
		if(null != DATA_IDS && !"".equals(DATA_IDS)){
			...
			equhighpcService.deleteAll(ArrayDATA_IDS);
			pd.put("msg", "ok");
		}else{
			pd.put("msg", "no");
		}
	...
	public ModelAndView exportExcel() throws Exception{
		logBefore(logger, Jurisdiction.getUsername()+"导出EquHighPC到excel");
		...
		List<PageData> varOList = equhighpcService.listAll(pd);
		...
		for(int i=0;i<varOList.size();i++){
			...
			vpd.put("var13", varOList.get(i).getString("IF_INTO_LINE"));	    //13
			vpd.put("var14", varOList.get(i).getString("CREATOR"));	    //14
			vpd.put("var15", varOList.get(i).getString("CREATE_TIME"));	    //15
			vpd.put("var16", varOList.get(i).getString("UPDATER"));	    //16
			vpd.put("var17", varOList.get(i).getString("UPDATE_TIME"));	    //17
			vpd.put("var18", varOList.get(i).get("DELETED").toString());	//18
			varList.add(vpd);
		}
