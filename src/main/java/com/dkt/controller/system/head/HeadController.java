package com.dkt.controller.system.head;
...
import com.dkt.service.system.appuser.AppuserManager;
import com.dkt.service.system.fhsms.FhsmsManager;
import com.dkt.service.system.user.UserManager;
import com.dkt.service.system.userphoto.UserPhotoManager;
import com.dkt.util.AppUtil;
import com.dkt.util.Const;
import com.dkt.util.Jurisdiction;
import com.dkt.util.PageData;
import com.dkt.util.SmsUtil;
import com.dkt.util.Tools;
import com.dkt.util.Watermark;
import com.dkt.util.mail.SimpleMailSender;
...
public class HeadController extends BaseController {
	...
	public ModelAndView saveSys() throws Exception{
		...
		Tools.writeFile(Const.EMAIL,pd.getString("SMTP")+",dkt,"+pd.getString("PORT")+",dkt,"+pd.getString("EMAIL")+",dkt,"+pd.getString("PAW"));	//写入邮件服务器配置
		Tools.writeFile(Const.SMS1,pd.getString("SMSU1")+",dkt,"+pd.getString("SMSPAW1"));	//写入短信1配置
		Tools.writeFile(Const.SMS2,pd.getString("SMSU2")+",dkt,"+pd.getString("SMSPAW2"));	//写入短信2配置
		mv.addObject("msg","OK");
	...
	public ModelAndView saveSys2() throws Exception{
		...
		PageData pd = new PageData();
		pd = this.getPageData();
		Tools.writeFile(Const.FWATERM,pd.getString("isCheck1")+",dkt,"+pd.getString("fcontent")+",dkt,"+pd.getString("fontSize")+",dkt,"+pd.getString("fontX")+",dkt,"+pd.getString("fontY"));	//文字水印配置
		Tools.writeFile(Const.IWATERM,pd.getString("isCheck2")+",dkt,"+pd.getString("imgUrl")+",dkt,"+pd.getString("imgX")+",dkt,"+pd.getString("imgY"));	//图片水印配置
		Watermark.fushValue();
		mv.addObject("msg","OK");
		mv.setViewName("save_result");
		return mv;
	...
	public ModelAndView saveSys3() throws Exception{
		...
		PageData pd = new PageData();
		pd = this.getPageData();
		String winStr = pd.getString("WIMIP")+",dkt,"+pd.getString("WIMPORT")+",dkt,";								//即时通讯配置
		String olStr = pd.getString("OLIP")+",dkt,"+pd.getString("OLPORT")+",dkt,"+pd.getString("FHsmsSound")+",dkt,";	//站内信配置
		String videoStr = pd.getString("VIDEOIP")+",dkt,"+pd.getString("VIDEOPORT");									//视频弹幕配置
	...
	public ModelAndView saveSys4() throws Exception{
		...
		Tools.writeFile(Const.LOGINEDIT,pd.getString("isZhuce")+",dkt,"+pd.getString("isMusic"));	//登录页面配置
		mv.addObject("msg","OK");
		mv.setViewName("save_result");
		return mv;
	}
