package com.dkt.controller.system.createcode;
...
import com.dkt.service.system.createcode.CreateCodeManager;
import com.dkt.util.AppUtil;
import com.dkt.util.DateUtil;
import com.dkt.util.DelAllFile;
import com.dkt.util.FileDownload;
import com.dkt.util.FileZip;
import com.dkt.util.Freemarker;
import com.dkt.util.Jurisdiction;
import com.dkt.util.PageData;
import com.dkt.util.PathUtil;
...
public class CreateCodeController extends BaseController {
	...
	public void proCode(HttpServletResponse response) throws Exception{
		...
		for(int i=0; i< zindex; i++){
			fieldList.add(pd.getString("field"+i).split(",dkt,"));	//属性放到集合里面
		}
...
//		Freemarker.printFile("sqlserver_SQL_Template.ftl", root, "sqlserver数据库脚本/"+tabletop+objectName.toUpperCase()+".sql", filePath, ftlPath);
	...
	public void save(PageData pd) throws Exception{
		...
		pd.put("TABLENAME", pd.getString("tabletop")+",dkt,"+pd.getString("objectName").toUpperCase());	//表名
		pd.put("FIELDLIST", pd.getString("FIELDLIST"));		//属性集合
		pd.put("CREATETIME", DateUtil.getTime());			//@date
