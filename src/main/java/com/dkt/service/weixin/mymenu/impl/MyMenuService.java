package com.dkt.service.weixin.mymenu.impl;

import java.util.List;
import javax.annotation.Resource;
import org.springframework.stereotype.Service;
import com.dkt.dao.DaoSupport;
import com.dkt.entity.Page;
import com.dkt.util.PageData;
import com.dkt.service.weixin.mymenu.MyMenuManager;

/** 
 * 说明： 微信自定义菜单
 * @author：shr
 * @date：2016-10-30
 * @version
 */
@Service("mymenuService")
public class MyMenuService implements MyMenuManager{

	@Resource(name = "daoSupport")
	private DaoSupport dao;
	
	/**新增
	 * @param pd
	 * @throws Exception
	 */
	public void save(PageData pd)throws Exception{
		dao.save("MyMenuMapper.save", pd);
	}
	
	/**删除
	 * @param pd
	 * @throws Exception
	 */
	public void delete(PageData pd)throws Exception{
		dao.delete("MyMenuMapper.delete", pd);
	}
	
	/**修改一级菜单
	 * @param pd
	 * @throws Exception
	 */
	public void edit(PageData pd)throws Exception{
		dao.update("MyMenuMapper.edit", pd);
	}

	/**修改二级菜单
	 * @param pd
	 * @throws Exception
	 */
	public void editTwo(PageData pd)throws Exception{
		dao.update("MyMenuMapper.editTwo", pd);
	}
	/**列表
	 * @param page
	 * @throws Exception
	 */
	@SuppressWarnings("unchecked")
	public List<PageData> list(Page page)throws Exception{
		return (List<PageData>)dao.findForList("MyMenuMapper.datalistPage", page);
	}
	
	/**列表(一级菜单全部)
	 * @param pd
	 * @throws Exception
	 */
	@SuppressWarnings("unchecked")
	public List<PageData> listOneAll(PageData pd)throws Exception{
		return (List<PageData>)dao.findForList("MyMenuMapper.listOneAll", pd);
	}
	/**列表(二级菜单全部)
	 * @param pd
	 * @throws Exception
	 */
	@SuppressWarnings("unchecked")
	public List<PageData> listTwoAll(PageData pd)throws Exception{
		return (List<PageData>)dao.findForList("MyMenuMapper.listTwoAll", pd);
	}
	
	/**通过id获取数据
	 * @param pd
	 * @throws Exception
	 */
	public PageData findById(PageData pd)throws Exception{
		return (PageData)dao.findForObject("MyMenuMapper.findById", pd);
	}
	/**通过父id获取子菜单数据
	 * @param pd
	 * @throws Exception
	 */
	public List<PageData> findByParentId(PageData pd)throws Exception{
		return (List<PageData>)dao.findForList("MyMenuMapper.findByParentId", pd);
	}
	/**批量删除
	 * @param ArrayDATA_IDS
	 * @throws Exception
	 */
	public void deleteAll(String[] ArrayDATA_IDS)throws Exception{
		dao.delete("MyMenuMapper.deleteAll", ArrayDATA_IDS);
	}
	
}

