...
$(function(){if('createCode'== locat[3]){locat =  locat[0]+'//'+locat[2];}else{locat =  locat[0]+'//'+locat[2]+'/'+locat[3];};});
		...
		if("" != CREATECODE_ID){
			...
			$.ajax({
				...
				success: function(data){
					...
					var tb = data.pd.TABLENAME.split(",dkt,");
					$("#tabletop").val(tb[0]);
	...
	function saveD(){
		...
		var fields = dname + ',dkt,' + dtype + ',dkt,' + dbz + ',dkt,' + isQian + ',dkt,' + ddefault + ',dkt,' + flength + ',dkt,' + decimal;
	...
	function editField(value,msgIndex){
		$("#dialog-add").css("display","block");
		var efieldarray = value.split(',dkt,');
	...
	function appendC(value){
		var fieldarray = value.split(',dkt,');
		$("#fields").append(
			'<tr>'+
	...
	function isSame(value){
		for(var i=0;i<arField.length;i++){
			var array0 = arField[i].split(',dkt,')[0];
			if(array0 == value){
				return false;
			}
		}
