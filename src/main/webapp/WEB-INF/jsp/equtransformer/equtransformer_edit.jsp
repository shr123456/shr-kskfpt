					...
					<form action="equtransformer/${msg }.do" name="Form" id="Form" method="post">
						...
						<table id="table_report" class="table table-striped table-bordered table-hover">
							...
							<tr>
								...
								<td><input type="number" name="HIGH_RATED_CURRENT" id="HIGH_RATED_CURRENT" value="${pd.HIGH_RATED_CURRENT}" maxlength="32" placeholder="这里输入高压额定电流" title="高压额定电流" style="width:98%;"/></td>
							</tr>
							<tr>
								<td style="width:75px;text-align: right;padding-top: 13px;">低压额定电流:</td>
								<td><input type="number" name="LOW_RATED_CURRENT" id="LOW_RATED_CURRENT" value="${pd.LOW_RATED_CURRENT}" maxlength="32" placeholder="这里输入低压额定电流" title="低压额定电流" style="width:98%;"/></td>
							</tr>
							<tr>
								<td style="width:75px;text-align: right;padding-top: 13px;">高压额定电压:</td>
								<td><input type="number" name="HIGH_RATED_VOLTAGE" id="HIGH_RATED_VOLTAGE" value="${pd.HIGH_RATED_VOLTAGE}" maxlength="32" placeholder="这里输入高压额定电压" title="高压额定电压" style="width:98%;"/></td>
							</tr>
							<tr>
								<td style="width:75px;text-align: right;padding-top: 13px;">低压额定电压:</td>
								<td><input type="number" name="LOW_RATED_VOLTAGE" id="LOW_RATED_VOLTAGE" value="${pd.LOW_RATED_VOLTAGE}" maxlength="32" placeholder="这里输入低压额定电压" title="低压额定电压" style="width:98%;"/></td>
							</tr>
							<tr>
							...
							<tr>
								...
								<td><input type="text" name="CAPACITY" id="CAPACITY" value="${pd.CAPACITY}" maxlength="20" placeholder="这里输入容量" title="容量" style="width:98%;"/></td>
							</tr>
							<tr>
...
<!-- /.main-container -->
	...
	<script type="text/javascript" src="static/js/jquery.tips.js"></script>
		...
		function save(){
			...
			if($("#HIGH_RATED_CURRENT").val()==""){
				$("#HIGH_RATED_CURRENT").tips({
					side:3,
		            msg:'请输入高压额定电流',
		            bg:'#AE81FF',
		            time:2
		        });
				$("#HIGH_RATED_CURRENT").focus();
			return false;
			}
			if($("#LOW_RATED_CURRENT").val()==""){
				$("#LOW_RATED_CURRENT").tips({
					side:3,
		            msg:'请输入低压额定电流',
		            bg:'#AE81FF',
		            time:2
		        });
				$("#LOW_RATED_CURRENT").focus();
			return false;
			}
			if($("#HIGH_RATED_VOLTAGE").val()==""){
				$("#HIGH_RATED_VOLTAGE").tips({
					side:3,
		            msg:'请输入高压额定电压',
		            bg:'#AE81FF',
		            time:2
		        });
				$("#HIGH_RATED_VOLTAGE").focus();
			return false;
			}
			if($("#LOW_RATED_VOLTAGE").val()==""){
				$("#LOW_RATED_VOLTAGE").tips({
					side:3,
		            msg:'请输入低压额定电压',
		            bg:'#AE81FF',
		            time:2
		        });
				$("#LOW_RATED_VOLTAGE").focus();
			return false;
			}
			if($("#MAKE_TIME").val()==""){
				$("#MAKE_TIME").tips({
