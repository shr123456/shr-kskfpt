									...
									<c:when test="${not empty varList}">
										<c:if test="${QX.cha == 1 }">
											<c:forEach items="${varList}" var="var" varStatus="vs">
												<tr>
													<%--<td class='center'>--%>
													<%--<label class="pos-rel"><input type='checkbox' name='ids' value="${var.POWEROUTAGES_ID}" class="ace" /><span class="lbl"></span></label>--%>
													<%--</td>--%>
													...
													<td class='center'>${var.CONTENT}</td>
														<c:if test="${var.TAG==0}">
															<td class='center'><a class="btn btn-sm btn-no" onclick="editSave(1,${var.POWEROUTAGES_ID});">未确认</a></td>
														</c:if>
														<c:if test="${var.TAG==1}">
															<td class='center'><a class="btn btn-sm btn-warning" onclick="editSave(2,${var.POWEROUTAGES_ID});">未解除</a></td>
														</c:if>
														<c:if test="${var.TAG==2}">
...
<script type="text/javascript">
    ...
    function editSave(num,id){
        var param={};
        param.num=num;
        param.POWEROUTAGES_ID=id;
        if(confirm("是否确定修改状态")) {
            $.ajax({
                url:"${path}/poweroutages/editTag.do",
                type:"post",
                dataType:"json",
                data:param,
